import { TrainingSummary } from 'chess-tree'
...
const TrainingCard: React.FC<TrainingCardProps> = ({training, width, onRestart, onDelete}) => {
    ...
    const content = <Fragment>
        ...
        />
        <CardContent>
            <Typography variant="body1" noWrap={true}>{ training.name }</Typography>
            <Typography variant="body2" pb={1} noWrap={true}>{training.linesTrained} lines trained</Typography>
    ...
    return <Card sx={{width: width, height: "400px"}}>
        ...
        <Menu open={menuAnchor !== null} anchorEl={menuAnchor} onClose={() => setMenuAnchor(null)}>
            ...
            <MenuItem onClick={onClickDelete}>Remove</MenuItem>
        </Menu>
    </Card>
}
